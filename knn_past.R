...
#' @return The predicted value as "fitted", index of all selected neighbors,
#' mean attribute and condidence intervals as NAs, forecast residuals, all
#' calculated distances to previouds elements, index used as initial and
#' accumulated distances to k-neighbors of each forecasted element. 
#' Also used kNN model.
#' @examples
...
knn_past <- function(y, k, d, initial = NULL, distance = "euclidean", weight =
  ...
  for (j in 2:(n - initial + 1)) {
    ...
    knn_dists[prediction_index] <- sum(distances_col[k_nn])/k
