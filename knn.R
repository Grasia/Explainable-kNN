...
#' @return The predicted value as "mean", index of selected neighbors,
#' fitted attribute and condidence intervals as NAs, forecast residuals and all
#' calculated distances to previouds elements. Also used kNN model.
#' @examples
...
knn <- function(y, k = 1:50, d = 1:50, initial = NULL, distance = "euclidean",
                ...
                threads = 1) {
    if (any(class(y) == "kNN")) {
        message("kNN model provided, simple prediction carried out",
                immediate. = TRUE)
        knn_forecast(y)
    }
    else if (length(k) == 1 && length(d) == 1) {
        message(paste0("k and d are single integers: supposing simple ",
                       "prediction is required"), immediate. = TRUE)
        knn_forecast(y = y, k = k, d = d, distance = distance, weight = weight,
                     v = v, threads = threads)
    }
    else {
        message(paste0("Beginning parameter search process. This may take a ",
